...
from camouflage import ScalingCamouflage
...
class DatasetBD(Dataset):
    def __init__(self, opt, full_dataset, inject_portion, transform=None, mode="train", device=torch.device("cuda"),
                 distance=1):
        if opt.cam_maps_address is not None:
            self.cam_maps = pd.read_csv(opt.cam_maps_address).values
            root_data_path = './data/poison-50-ImageNet-n01537544/'
            for i in range(0, len(self.cam_maps[:, 3])):
                self.cam_maps[i][3] = root_data_path + self.cam_maps[i][3]
        ...
        for i in tqdm(range(len(dataset))):
            ...
            if target_type == 'all2one':
                ...
                if mode == 'train':
                    ...
                    if i in perm:
                        # select trigger
                        img = self.selectTrigger(img, width, height, distance,
                                                 trig_w, trig_h, trigger_type,
                                                 self.cam_maps[int(i/len(dataset)*len(self.cam_maps))])
    ...
    def selectTrigger(self, img, width, height, distance, trig_w, trig_h, triggerType, cam_map):

        assert triggerType in ['squareTrigger', 'gridTrigger', 'fourCornerTrigger', 'randomPixelTrigger',
                               'signalTrigger', 'trojanTrigger', 'dynamic', 'camouflageTriggerCircle']
        ...
        elif triggerType == 'camouflageTriggerCircle':
            img = self._camouflageTriggerCircle(img, width, height, distance, trig_w, trig_h, cam_map)
    ...
    def _camouflageTriggerCircle(self, img, width, height, distance, trig_w, trig_h, cam_map):

        x2, y2, r2, cam_img_name = cam_map

        img_ = img

        # use cv2.imread, need to change the order of channels
        # cam_img_ = cv2.imread(cam_img_name)
        # cam_img_ = cv2.cvtColor(cam_img_, cv2.COLOR_BGR2RGB)

        # use Image.open
        cam_img_ = np.array(Image.open(cam_img_name))

        sourceImg = cv2.resize(cam_img_, (1800, 1300), interpolation=cv2.INTER_LINEAR)
        scaleImg = cv2.resize(sourceImg, (224, 224), interpolation=cv2.INTER_LINEAR)

        # print(cam_img_.shape)
        # cam_img_ = cam_img.permute((1, 2, 0))

        img_shape_1_, img_shape_2_, img_shape_3_ = img_.shape
        img_shape_ = (img_shape_1_, img_shape_2_)

        targetImg = cv2.resize(img_, (224, 224))

        # print(img_.shape)
        # print(cam_img_.shape)

        for j in range(x2 - r2, x2 + r2):
            for i in range(y2 - r2, y2 + r2):
                pe = (j - x2) ** 2 + (i - y2) ** 2
                if pe >= r2 ** 2:
                    continue

                # targetImg[i][j] = (1.0-math.sqrt(pe)/r2)*scaleImg[i][j] + math.sqrt(pe)/r2*targetImg[i][j]
                targetImg[i][j] = scaleImg[i][j]
